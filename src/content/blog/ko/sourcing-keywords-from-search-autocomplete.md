...
그런데 **이 파이프라인에서 0개는 이미 정상 결과**입니다. 네이버는 아무도 안 치는 조합(`매트리스 치료` 같은)에 빈 배열을 정직하게 줍니다. 고장이 아니라 "이 말은 수요가 없다"는 판정을 공짜로 얻는 겁니다.
...
바꾸기 전에는 **같은 버튼이 25.1초**였습니다. LLM 두 번이 끝나야 목록이 떴으니까요.
...
키워드 발굴을 25.1초에서 0.2초로 줄인 결정은 단순한 API 교체가 아니었습니다. 자동완성 엔드포인트가 언제든 바뀔 수 있다는 전제를 받아들이고, 파싱 검증·이중 엔진·캐시·저장 시점 변경을 함께 둔 결과였습니다. 그래서 이 글의 결론은 "빠른 엔드포인트를 찾았다"보다, **불안정한 외부 입력을 제품 경로에 넣을 때 어떤 방어선을 남길지 먼저 정했다**에 가깝습니다.
