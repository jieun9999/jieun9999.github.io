...
subtitle: "진짜 병목은 LLM이 아니라 크롤링이었다"
pubDate: 2026-06-29
...
> **구현 디테일(논블로킹 종료):** fresh-profile Chrome은 `--dump-dom` 이후 바로 종료되지 않아(백그라운드 작업 때문에) 요청마다 약 45초 타임아웃이 걸렸습니다. 해법: stdout을 스트리밍하다가 `</html>`가 나타나는 순간 프로세스를 죽이고, 호출마다 프로필 잠금을 정리하며, 전역 락으로 직렬화합니다. 응답이 3–5초로 떨어졌습니다.
