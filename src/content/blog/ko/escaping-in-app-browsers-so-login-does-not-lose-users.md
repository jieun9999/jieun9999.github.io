...
subtitle: "인앱 브라우저에서 간편로그인 버튼이 사라지는 문제를 확인했습니다. 에러 로그에 남지 않는 이탈에 대응하기 위해 브라우저를 감지하고, 외부 브라우저 전환과 폴백 경로를 마련했습니다."
pubDate: 2026-09-08
tags:
  [
    "in-app-browser",
...
| Threads 인앱 브라우저 링크 | 계정·비밀번호 입력 폼**만** 남습니다      |

![카카오 로그인 화면 두 장. 왼쪽은 Threads 인앱 브라우저(상단에 X·뒤로가기·⋯ 버튼과 'Threads' 표시)로 아이디·비밀번호 폼만 보이고, 오른쪽은 Safari 주소창 아래 '카카오톡으로 로그인' 버튼이 맨 위에 있습니다](/images/kakao-login-in-app-vs-safari.webp)
...
더 나쁜 건 **이 손실이 어디에도 안 남는다는 점**입니다. 서버는 정상이고, 500도 없고, 로그인 실패 로그조차 안 쌓입니다. 시도를 안 하고 나가기 때문입니다. 개발자는 크롬에서 테스트하니 영영 못 봅니다.
...
카카오 입장에서는 합리적인 폴백입니다. 문제는 그 폴백의 비용이 **제가 담당하던 서비스의 전환율 손실로 돌아온다는 것**뿐입니다.
...
셋 다 "쉬운 로그인"의 장점을 약화합니다. 이미 로그인돼 있어야 할 사용자가 로그아웃 상태로 시작하고, 저장해둔 비밀번호는 안 뜨고, 앱 전환 버튼은 사라집니다.
...
실사용자 반응은 이랬습니다. **사파리인지 크롬인지 관심 없습니다. 버튼을 누르면 그냥 넘어가면 됩니다.**
...
카카오톡 알림톡 안에서 링크를 눌러도 같은 로직이 적용됩니다(`KAKAOTALK` 토큰). 스레드뿐 아니라 카카오톡 자기 자신의 인앱 브라우저도 같은 방식으로 처리됩니다.
