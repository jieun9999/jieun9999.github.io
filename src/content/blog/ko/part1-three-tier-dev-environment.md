...
subtitle: "4시간에 1.2GB → 3.7GB로 자라는 dev 서버가 알려준 것"
pubDate: 2026-09-15
tags:
...
`next dev`는 서버 컴포넌트와 라우트 핸들러를 포함합니다. 앞선 글에서 admin을 BFF라고 불렀던 그 계층입니다. **백엔드 절반이 서버에서 돕니다.** 맥북은 픽셀만 받습니다.

그래서 2분할이 아니라 "브라우저 / 개발 프로세스 전부 / 운영 데이터" 3단입니다. 이렇게 세고 나서야 셋째 단이 문제라는 게 보였습니다.
...
대안은 콜백 오리진을 동적으로 만드는 것인데, OAuth 공급자에 사전 등록이 필요한 구조라 결국 등록 목록을 관리해야 합니다. 복잡도가 더 크다고 보고 감수했습니다. 대신 이 제약을 기동 스크립트 출력에 적어뒀습니다. 사람이 기억하지 않아도 되게요.
...
마지막 방어선이 "사람의 주의"인 구조에 그걸 올리면 방어선이 사라집니다. 게다가 실패하면 재실행하니 사고가 한 번이 아니라 여러 번 납니다.
...
2번이 제일 중요합니다. 운영 터널과 e2e 스택이 같은 `API_BASE`를 쓰는 순간 사고가 납니다. 포트를 따로 주고, e2e 쪽은 운영 터널이 떠 있든 없든 무관하게 도는 구조여야 합니다.
...
회선이 느린 곳에서 일한다면 HMR 왕복이 매번 네트워크를 타는 게 거슬릴 테니까요. 저희가 이쪽을 고른 이유는 단순합니다. 코딩 에이전트를 서버에서 돌리고 있어서 **코드와 에이전트가 같은 자리에 있어야** 했습니다.
