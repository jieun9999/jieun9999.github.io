...
Astro는 **GFM**으로 렌더링한다. 아래 문자들은 본문에서 예상 밖으로 서식으로 해석되니 주의.

- **🔴 숫자 범위에 물결표 `~`를 쓰지 않는다.** GFM은 `~텍스트~`를 **취소선**(`<del>`)으로
  해석한다. 한 줄에 `~`가 두 개 있으면(예: `10~16자/초 … 2.5~22초`) 그 사이가 통째로
  줄이 그어진다. 범위는 **en-dash `–`**(`10–16`, `20–45분`)로 쓴다. EN 글도 이미 이 표기.
- 그 밖에 문장 중간의 `*`·`_`(강조), `` ` ``(코드), `|`(표), 줄머리 `#`·`-`·`>` 도
  의도치 않게 서식이 될 수 있다. 리터럴로 보이려면 `\` 로 이스케이프.
- **확인법**: `npm run build` 후 `dist/<lang>/blog/<slug>/index.html`에서 의도 안 한
  `<del>`/`<em>` 등이 없는지 grep. 특히 숫자·기호가 많은 글은 배포 전 렌더 결과를 본다.
